import { getSkillCategory } from '@/api/skill-category.api';
...
export default function SkillCategorySelector({ selected, multiple, onSelect, children }: Props) {
  const [name, setName] = useState('');
  const [open, setOpen] = useState(false);

  const { data, isLoading } = useQuery({
    queryKey: ['skill-categories', name],
    queryFn: () =>
      getSkillCategory({
        name,
        page: 1,
