...
const InputOTPSlot = React.forwardRef<React.ElementRef<'div'>, React.ComponentPropsWithoutRef<'div'> & { index: number }>(({ index, className, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={cn('relative flex h-9 w-9 items-center justify-center text-sm shadow-sm transition-all rounded-2xl', isActive && 'z-10 ring-1 ring-ring', className)} {...props}>
      {char}
      {hasFakeCaret && (
